...
/**
 * Nabor for monochrome effect
 *
 * @author maeda6uiui
 */
public class MonochromeEffectNabor extends FullScreenEffectNabor {
    public MonochromeEffectNabor(VkDevice device, List<URL> vertShaderResources, List<URL> fragShaderResources) {
        super(device, vertShaderResources, fragShaderResources);
    }

    @Deprecated
    public MonochromeEffectNabor(VkDevice device, URL vertShaderResources, URL fragShaderResource) {
        this(device, List.of(vertShaderResources), List.of(fragShaderResource));
    }
    ...
    protected void createUniformBuffers(int descriptorCount) {
        ...
        var uboInfos = BufferUtils.createUBOBuffers(
                device, descriptorCount, MonochromeEffectInfoUBO.SIZEOF);
        uboInfos.forEach(v -> {
    ...
    protected void createDescriptorSetLayoutSet0() {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VkDescriptorSetLayoutBinding effectInfoUBOLayoutBinding = uboLayoutBindings.get(0);
            effectInfoUBOLayoutBinding.binding(0);
            effectInfoUBOLayoutBinding.descriptorCount(1);
            effectInfoUBOLayoutBinding.descriptorType(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
            effectInfoUBOLayoutBinding.pImmutableSamplers(null);
            effectInfoUBOLayoutBinding.stageFlags(VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    protected void createDescriptorPoolSet0(int descriptorCount) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VkDescriptorPoolSize effectInfoUBOPoolSize = uboPoolSizes.get(0);
            effectInfoUBOPoolSize.type(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
            effectInfoUBOPoolSize.descriptorCount(descriptorCount);
