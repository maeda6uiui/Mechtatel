...
import com.github.maeda6uiui.mechtatel.core.vulkan.ubo.postprocessing.ParallelLightUBO;
...
public class ParallelLightNabor extends PostProcessingNabor {
    ...
    public ParallelLightNabor(VkDevice device, List<URL> vertShaderResources, List<URL> fragShaderResources) {
    ...
    protected void updateDescriptorSet0(int descriptorCount, long commandPool, VkQueue graphicsQueue) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            lightUBOInfo.range(ParallelLightUBO.SIZEOF * MAX_NUM_LIGHTS);
