...
import com.github.maeda6uiui.mechtatel.core.vulkan.ubo.postprocessing.RoughWaterSurfaceUBO;
...
/**
 * Nabor for rough water surface
 *
 * @author maeda6uiui
 */
public class RoughWaterSurfaceNabor extends PostProcessingNabor {
    public RoughWaterSurfaceNabor(VkDevice device, List<URL> vertShaderResources, List<URL> fragShaderResources) {
    ...
    protected void createUniformBuffers(int descriptorCount) {
        ...
        var waterSurfaceUBOInfos = BufferUtils.createUBOBuffers(
                device, descriptorCount, RoughWaterSurfaceUBO.SIZEOF);
    ...
    protected void createDescriptorSetLayoutSet0() {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VkDescriptorSetLayoutBinding waterSurfaceUBOLayoutBinding = uboLayoutBindings.get(1);
            waterSurfaceUBOLayoutBinding.binding(1);
            waterSurfaceUBOLayoutBinding.descriptorCount(1);
            waterSurfaceUBOLayoutBinding.descriptorType(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
            waterSurfaceUBOLayoutBinding.pImmutableSamplers(null);
            waterSurfaceUBOLayoutBinding.stageFlags(VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    protected void createDescriptorPoolSet0(int descriptorCount) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VkDescriptorPoolSize waterSurfaceUBOPoolSize = uboPoolSizes.get(1);
            waterSurfaceUBOPoolSize.type(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
            waterSurfaceUBOPoolSize.descriptorCount(descriptorCount);
    ...
    protected void updateDescriptorSet0(int descriptorCount, long commandPool, VkQueue graphicsQueue) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            waterSurfaceUBOInfo.range(RoughWaterSurfaceUBO.SIZEOF);
