...
public class RoughWaterSurfaceTest extends Mechtatel {
    private static final Logger logger = LoggerFactory.getLogger(RoughWaterSurfaceTest.class);

    public RoughWaterSurfaceTest(MttSettings settings) {
        super(settings);
        this.run();
    }
    ...
    public void onCreate(MttWindow window) {
        ...
        //the scene underneath is barely visible through the churned surface and foam.
        PostProcessingProperties ppProp = mainScreen.getPostProcessingProperties();
        RoughWaterSurface water = ppProp.roughWaterSurface;
        ...
        water.setWaveAmplitude(0.4f);
        water.setWaveFrequency(1.0f);
        water.setWaveSpeed(1.8f);
        water.setChoppiness(2.0f);
        water.setFoamColor(new Vector3f(0.9f, 0.95f, 1.0f));
        water.setFoamThreshold(0.6f);
        water.setFoamIntensity(1.0f);
        water.setSpecularStrength(1.2f);
